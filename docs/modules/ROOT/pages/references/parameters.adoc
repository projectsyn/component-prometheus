...
This parameter configures the `alert-routing-discovery` addon which enables users to easily implement dynamic alert routing based on each Commodore component's https://syn.tools/syn/explanations/argocd-multitenancy.html[Project Syn team].
...
== `instances.*.prometheus.remoteWrite`
...
prometheus:
  remoteWrite:
    myRemoteWriteTarget:
      url: http://mymetrics.com/api/v1/push
      headers:
        "X-Scope-OrgID": best-org-ever
      writeRelabelConfigs:
        - action: keep
...
Can be used to supply additional remoteWrite targets to Prometheus.
All timeseries listed under `writeRelabelConfigs.timeseries` are automatically joined into a regular expression that will match either of the provided timeseries.
The resulting expression is used as the `regex` parameter in the final remote write configuration.
